...
let get_gl_include_file_name api version = match api, version with
  ...
  | GLES2_API, "3.0" -> "GLES2/gl3.h"
  | GLES2_API, "3.1" -> "GLES2/gl31.h"
  | GLES2_API, "3.2" -> "GLES2/gl32.h"
...
" (string_of_api api) version (get_gl_include_file_name api version);
    ...
    |> List.iter (fun enum ->
           fprintf ml_out "  | %s : [<`%s] enum\n" (to_pascal_case enum.ename) (String.concat "|`" enum.groups);
           ...
           if not needs_block_allocation then (
             List.iter (fun param ->
                 ...
                 | Array ml_type ->
                    ...
                    let element = match ml_type with
                      | Int | Type _ -> sprintf "Int_val(Field(%s, i))" pname
                      | Bool -> sprintf "Bool_val(Field(%s, i))" pname
                      | Float -> sprintf "Double_val(Field(%s, i))" pname
                      | String -> sprintf "String_val(Field(%s, i))" pname
                      | Int64 -> sprintf "Int64_val(Field(%s, i))" pname
                      | Enum _ -> sprintf "gl_enums[Int_val(Field(%s, i))]" pname
                      | List _ -> sprintf "glbitfield_of_enum_list(gl_enums, Field(%s, i))" pname
                      | Bigarray _ -> sprintf "Caml_ba_data_val(Field(%s, i))" pname
                      | _ -> "NULL"
                    in

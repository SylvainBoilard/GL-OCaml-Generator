CAMLprim value caml_glGetActiveAttrib(value program, value index)
{
    CAMLparam0();
    GLsizei length;
    GLint size;
    GLenum type;
    GLint bufSize;
    glGetProgramiv(Int_val(program), GL_ACTIVE_ATTRIBUTE_MAX_LENGTH, &bufSize);
    GLchar name[bufSize];
    glGetActiveAttrib(Int_val(program), Int_val(index), bufSize, &length, &size, &type, name);
